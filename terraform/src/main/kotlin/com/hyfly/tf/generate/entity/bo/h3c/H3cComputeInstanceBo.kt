...
data class H3cComputeInstanceBo(
    ...
    var networks: List<H3cComputeInstanceNetworkBo>? = null,
) {
    /**
     * 将请求对象转换为业务对象
     *
     ...
     * @return 业务对象
     */
    fun fromReq(req: H3cComputeInstanceReq): H3cComputeInstanceBo {
        this.name = req.name
        ...
        req.networks?.let {
            val list = mutableListOf<H3cComputeInstanceNetworkBo>()

            val json = JSON.toJSONString(it)
            val reqList = JSON.parseArray(json, H3cComputeInstanceNetworkReq::class.java)

            for (networkReq in reqList) {
                val networkBo = H3cComputeInstanceNetworkBo()
